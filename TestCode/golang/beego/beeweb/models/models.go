...
func InitModels() {
	...
	setGithubCredentials(beego.AppConfig.String("github::client_id"),
		beego.AppConfig.String("github::client_secret")) //读取目录conf下的app.conf中的[github]部分的内容

	docLock = new(sync.RWMutex) //同步锁
...
func checkFileUpdates() error {
	...
	var trees = []*tree{
		{
			ApiUrl:   "github.com/beego/beedoc&" + githubCred,
			RawUrl:   "github.com/beego/beedoc",
